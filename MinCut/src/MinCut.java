...
public class MinCut {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<iter;i++){
			...
			Integer counter;
			if((counter = stat.get(currMin))== null){
				counter = 0;
			}
			stat.put(currMin, counter+1);
		}
		System.out.println("Min: " + min + " stat: " + (stat.get( min )*100/iter) + "%" );
	}
	public static int[][] readmap(){
		LinkedHashMap<Integer, List<Integer>> vertices = new LinkedHashMap<Integer, List<Integer>>();
		try{
			FileReader fr = new FileReader("/Users/mac/Desktop/kargerMinCut.txt");
	...
	private static int minCut(Graph gr){
